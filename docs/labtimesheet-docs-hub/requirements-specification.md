...
| View Intern attendance | No | Yes | Own history only | Own history only |
| Decide leave/correction | No | Yes | No | No |
| Submit own leave/correction | No | No | If active Intern | If active Intern |
| Export authorized attendance/project reports | No | Authorized scope | Project scope | No detailed export |
...
| RPT-004 | Active Mentors may view detailed Intern attendance. Interns may view their own history. Admins have no Attendance-report scope or report dataset. Project Leaders do not receive another Intern's attendance merely because they lead a Project; as Interns, their Attendance scope remains their own history. |
...
| RPT-011 | An owning Mentor may request a Daily Project Work Report for all owned Projects or one selected owned Project. The global Daily entry remains available to Mentors. An active Intern sees a conditional Daily entry only when the server confirms that they currently lead at least one `PLANNED` or `ACTIVE` Project; one eligible Project may redirect directly to its locked report, while multiple eligible Projects show an authorized selector and no eligible Project returns `Project unavailable`. A current Project Leader may request HTML, XLSX, or PDF for one mandatory exact currently-led `PLANNED` or `ACTIVE` Project, for today or a permitted past Report date, including the whole retained Project history before the current leadership term. Leader HTML/XLSX/PDF requests re-authorize the exact `projectId`, and a valid selected date survives selection or redirection. Admins have no Daily-report scope: their navigation is absent and Daily HTML/XLSX/PDF requests retain the non-disclosing `Project unavailable` response. Admin Attendance and Project/Task HTML/XLSX/PDF requests are denied with the authenticated access-denied response before target/Project option resolution, Attendance/Task/work-log reads, dataset construction, or exporter invocation. Ordinary Interns without current leadership, former Leaders, Leaders of another Project, completed Projects, missing `projectId` requests, and guessed IDs shall be denied before Attendance context, Task reads, or exporter invocation. |
...
| AC-RPT-002 | RPT-002–RPT-005 | An Intern requests another Intern's Attendance, an active Mentor requests authorized detailed Attendance, an owning Mentor/current Leader requests authorized Project/Task detail, an ordinary member requests broader detail, and an Admin requests Attendance or Project/Task HTML/XLSX/PDF | Intern scope remains own history; active Mentor and authorized owning-Mentor/current-Leader scopes remain available; ordinary members receive aggregate-only Project/Task results; Admin requests return authenticated access denial before target/Project option resolution, Attendance/Task/work-log reads, dataset construction, or exporter invocation. |
