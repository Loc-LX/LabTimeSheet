...
| RPT-011 | A Mentor may request a Daily Project Work Report for owned Projects, and an Admin may request one for Admin-authorized Projects, for one non-future Report date optionally scoped to a Project. It groups retained Task work logs logged on the selected Report date by Project and author, includes current Task status and retained descriptions regardless of attendance/calendar classification, and includes soft-deleted Task logs marked as deleted. |
...
| AC-RPT-004 | RPT-011–RPT-012 | Mentor/Admin requests Daily Project Work Report for a past, current, policy non-workday, global day off, empty scope, and soft-deleted Task | Authorized retained logs remain grouped and described with current status/deletion marker; date labels never filter valid Task work; empty states and access denial follow scope. |
