...
- Independent review so far covered the five claims flagged in `D38` and `D39` and the errors found around them, which are corrected. A full review of `D34`–`D37` has not been done. Nothing from this branch has been pushed or merged into main.
...
- `D39` records a class, not a single omission: four predicates of `V1__baseline.sql` list statuses that the rules widen, and widening only the status column leaves them wrong. They are the invitation resolution codes (`PROJECT_CANCELLED`), the leave overlap exclusion, which lets overdue leave overlap without failing any existing test, and the decision checks of leave requests and corrections. `DB-018` now says undecided rows carry no decision and a cancelled leave request carries its approval; `AC-DB-010` and `AC-DB-011` cover them. Stored `CANCELLED` rows with no decision time become `WITHDRAWN`, a reading from the code that must be confirmed against the data.
