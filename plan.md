...
**Last updated:** 2026-09-21 · **Branch:** `work/fix/docs/module-feature-specs`.
...
`backup/pre-feature-split-20260920`. The subsequent D34 restructuring is local and uncommitted.
...
| Specification | Eight module contracts and 27 feature SPECs; [entry map](.sdd/specs/README.md). D34 organizes workflows; D35 settles Task creation/unblocking, cancelled-Project internship readiness and current-session logout. D36 additionally excludes soft-deleted Tasks from internship readiness. D37 settles assignee non-deleted Task transitions, aligns DB-011 and DB-012 status constraints with V1__baseline.sql, and defines complete invitation and exit-request state transition tables. D38 settles the account state machine, the email delivery states and password-reset eligibility, and removes the parallel `feature-*` document tree. Current catalogue: 310 rules and 163 scenarios. §22.3 is empty, so §22.2's condition on it is met; acceptance by the authority in §1.1 is still required. |
| Constitution | Locked, version `2.0.2`. |
| Technical plans (`PLAN.md`) | The platform technical plan, now at `.sdd/specs/platform/PLAN.md`, is version `1.0`; its line 108 contradicts line 230 on who resolves scope, and its §3 is superseded by `D32`. Revise it in the plan phase. New technical plans belong beside their feature SPECs and inherit MODULE.md; no empty plans were created. The cross-module architecture plan location remains open. |
...
- Legacy SPEC paths remain compatibility indexes; root maps, template, document contract tests and analyzer input discovery follow the new structure. No Java, migration or dependency changed. The JS test under `src/test/` did change.
...
- Independent review of D34 remains pending. The restructuring has not been committed, pushed or merged into main.
...
- `D38` closes the last entries of §22.3. `ACC-014` names every account transition, `ACC-028` deactivates an account created in error while still pending, `ACC-029` reinstates a deactivated account, and `DB-022` holds the schema to the same set while tightening its timestamp pairings. `NOT-012` states the five email delivery states and their transitions. `SEC-015` states password-reset eligibility and forbids a reset from releasing an Admin's lock. Seven scenarios cover them, three of which complete password-management coverage. Account and delivery state transition tables are added, bringing the catalogue to 310 rules and 163 scenarios. `ACC-028`, `ACC-029` and `DB-022` need a migration and join the one `D32` already requires; no Java, schema or dependency changed here.
- `D39` records the one schema delta no document owned: `DB-011` requires nine invitation resolution codes and `V1__baseline.sql` accepts eight, so `PROJECT_CANCELLED` and `AC-PRJ-015` need the migration `D32` already requires. Every enumerated value in a `DB` rule was compared against both migrations; the other absent values are `D32`'s own new status names, so this was a single omission rather than a class. The rule text is unchanged.
...
- `D38` Part 1 removes the parallel `feature-*` document tree. Ninety-nine changelog entries were appended verbatim to the owning module's changelog under *History before the D34 feature split*, `feature-platform/PLAN.md` moved to `.sdd/specs/platform/PLAN.md`, and thirty-five successor links were retargeted. Deleting the sixteen files themselves is the one step still waiting on the maintainer.
...
| Permission to delete the sixteen tracked files of the eight `feature-*` folders. Every other part of `D38` Part 1 is done and the hierarchy check fails until they are gone | The maintainer | `D38`, this file |
