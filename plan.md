...
| Platform F1–F6 | Parts B (authorization) and C (schema change) of `platform/PLAN.md` were drafted on 22 September 2026, replacing version 1.0: B follows `R6` (the owning module resolves scope; the policy looks nothing up) and C expands in one migration, then contracts module by module with each module's code | Approval of parts B and C; both start after part A is done |
...
| Approval of parts B and C of `platform/PLAN.md`, drafted on 22 September 2026 on `work/fix/docs/platform-plan-bc` | The maintainer | `platform/PLAN.md` |
