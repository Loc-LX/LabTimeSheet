...
| `ARC-007` | No structural test proves Flyway is the only production schema authority; the catalog-shape test does not exclude another schema creator. | A structural test that fails when a production schema authority other than Flyway is configured or used. |
...
| `ARC-008` | Nothing in this repository can enforce it: `database-schema.sql` is not tracked here, as the specification header states. | Nothing, and that is the point. The rule records a completed one-time adaptation. `ARC-007` is what binds future schema work; its Flyway-only authority remains a known gap above. |
