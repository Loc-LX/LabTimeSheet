...
| Field | Specification |
|---|---|
| Primary actor(s) | Admin for detailed Intern Attendance reports only; Mentor; authorized Intern/Leader for their own/current scope. An Admin has no Project/Task report scope under `RPT-005` and no Daily Project Work Report scope under `RPT-011`. |
...
- `D1` was confirmed by the instructor on 14 September 2026: an Admin may view detailed Intern attendance, and for now sees every field a Mentor sees, which `RPT-004` lists. The instructor expects to remove fields from Admin later, so the plan must let that be changed in one place. Today it is decided in three: `SecurityConfiguration`, `AttendanceReportQueryService#authorize`, and the shared layout, none of them per field.
- **Open.** Whether the instructor's "full access" for Admin also covers the Project/Task and Daily reports, which `RPT-005` and `RPT-011` deny to Admin today.
- **Open, Daily Project Work Report.** The estimate, lifetime actual, forecast, and variance belong to the Task but repeat under every author who logged that day. Undecided: who the report is mainly for, and so whether it needs a Task view and an Intern view; whether Remaining effort should update as work continues, which `GOV-015` forbids today; and whether the forecast total stays a snapshot.
