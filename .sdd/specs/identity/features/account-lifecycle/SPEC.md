...
| DB-022 | THE schema SHALL constrain `app_users.account_status` to `PENDING_ACTIVATION`, `ACTIVE`, `LOCKED` or `DEACTIVATED`. THE schema SHALL require no password hash and no activation timestamp WHILE the status is `PENDING_ACTIVATION`; a non-blank password hash and an activation timestamp WHILE it is `ACTIVE` or `LOCKED`; and, WHILE it is `DEACTIVATED`, either a non-blank password hash with an activation timestamp or neither of them. THE schema SHALL let an activation timestamp be set only by the update that moves an account from `PENDING_ACTIVATION` to `ACTIVE`, and SHALL refuse every other update that sets, clears or changes it. THE schema SHALL require a lock timestamp WHILE the status is `LOCKED`, SHALL permit one WHILE it is `DEACTIVATED` only beside an activation timestamp, and SHALL refuse one in every other status. THE schema SHALL let a lock timestamp be set only by the update that moves an account from `ACTIVE` to `LOCKED`, cleared only by the update that moves it from `LOCKED` to `ACTIVE`, and SHALL refuse every other update that sets, clears or changes it, so that a lock carried into `DEACTIVATED` can only leave through `LOCKED`. THE schema SHALL require a deactivation timestamp exactly WHILE the status is `DEACTIVATED`. |
...
| AC-DB-009 | DB-022, ACC-014, ACC-016, ACC-028, ACC-029 | Each account status is written directly to the schema with every combination of password hash, activation timestamp, lock timestamp and deactivation timestamp, including a blank password hash. Then, bypassing the application: an update tries to clear and another to change the activation timestamp of an activated account in each status; one tries to set an activation timestamp on a pending account without moving it to `ACTIVE`; one moves a `DEACTIVATED` account holding a lock timestamp straight to `ACTIVE` while clearing that timestamp; one sets a lock timestamp on a `DEACTIVATED` account that has none, alone and while moving it to `LOCKED`; and one clears or changes the lock timestamp of a `DEACTIVATED` account in place | The schema accepts exactly the lawful combinations: neither hash nor activation timestamp for `PENDING_ACTIVATION`; both for `ACTIVE` and `LOCKED`; both or neither for `DEACTIVATED`; a lock timestamp while `LOCKED`, and while `DEACTIVATED` only beside an activation timestamp; a deactivation timestamp only while `DEACTIVATED`. Every one of the listed updates is refused even when application authorization is bypassed, so a lock carried into `DEACTIVATED` can leave only through `LOCKED`, while an update that rewrites a timestamp with its unchanged value is not treated as a change. |
