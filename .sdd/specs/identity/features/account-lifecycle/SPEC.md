...
| ACC-014 | THE system SHALL hold an account in exactly one of `PENDING_ACTIVATION`, `ACTIVE`, `LOCKED`, or `DEACTIVATED`. Lock, deactivation and reinstatement SHALL occur only through an explicit Admin action. THE system SHALL permit only these account transitions: `PENDING_ACTIVATION → ACTIVE` on first-password activation under `ACC-011`, `ACTIVE → LOCKED`, `LOCKED → ACTIVE`, `ACTIVE → DEACTIVATED`, `LOCKED → DEACTIVATED`, `PENDING_ACTIVATION → DEACTIVATED` under `ACC-028`, and `DEACTIVATED → ACTIVE` or `DEACTIVATED → PENDING_ACTIVATION` under `ACC-029`. WHERE any other transition is requested, THE system SHALL reject it and leave the account unchanged. |
...
| ACC-016 | WHILE an account is `DEACTIVATED`, THE system SHALL refuse authentication for it and SHALL keep its historical attribution visible. THE system SHALL NOT treat that state as permanent: an Admin MAY reinstate the account under `ACC-029`. |
...
| ACC-029 | WHILE an account is `DEACTIVATED`, THE system SHALL permit an Admin to reinstate it. WHEN a reinstated account holds an activation timestamp, THE system SHALL return it to `ACTIVE` without recreating its credentials. WHERE it holds none, THE system SHALL return it to `PENDING_ACTIVATION`, and a usable activation link SHALL require an explicit resend under `ACC-013`. THE system SHALL NOT restore an internship, a Project membership or a leadership term by reinstating an account. |
| DB-022 | THE schema SHALL constrain `app_users.account_status` to `PENDING_ACTIVATION`, `ACTIVE`, `LOCKED` or `DEACTIVATED`, and SHALL require the password hash and the activation timestamp to agree: both absent WHILE the status is `PENDING_ACTIVATION`, or WHILE it is `DEACTIVATED` and the account was never activated, and both present otherwise. THE schema SHALL require a lock timestamp exactly WHILE the status is `LOCKED`, and a deactivation timestamp exactly WHILE it is `DEACTIVATED`. |
...
| `PENDING_ACTIVATION` | set the first password through the activation link | `ACTIVE` | intended user | `ACC-011` |
...
| `ACTIVE`, `LOCKED` | deactivate | `DEACTIVATED` | Admin | `ACC-014`, `ACC-016`, `ACC-018` |
| `DEACTIVATED` | reinstate an account that was activated before | `ACTIVE` | Admin | `ACC-029` |
...
| AC-ACC-021 | ACC-013, ACC-014, ACC-029, ACC-024 | An Admin reinstates two `DEACTIVATED` accounts: one deactivated after activation, including one deactivated by internship withdrawal, and one deactivated while still pending; sign-in is then attempted for each before and after an activation resend | The previously activated account returns to `ACTIVE` and signs in with its existing credentials, which are not recreated. The never-activated account returns to `PENDING_ACTIVATION` and cannot sign in until the Admin resends activation and the user sets a first password. Neither reinstatement restores an internship, a Project membership or a leadership term, and the withdrawn internship stays withdrawn. |
| AC-DB-009 | DB-022, ACC-014, ACC-028 | Each account status is written directly to the schema with every combination of password hash, activation timestamp, lock timestamp and deactivation timestamp, including a `DEACTIVATED` row that holds an activation timestamp but no password hash | The schema accepts exactly the lawful combinations: both absent for `PENDING_ACTIVATION` and for a never-activated `DEACTIVATED` row, both present otherwise, a lock timestamp only while `LOCKED`, and a deactivation timestamp only while `DEACTIVATED`. Every half-populated row is rejected even when application authorization is bypassed. |
