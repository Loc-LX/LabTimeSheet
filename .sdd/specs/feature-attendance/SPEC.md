...
**Version:** 1.3.0 · **Owner:** Loc-LX · **Status:** APPROVED · **Date:** 2026-09-14
...
| COR-005 | WHILE a request is pending or overdue and its period is not finalized, THE system SHALL permit the Intern's responsible Mentor under `ACC-026` to approve or reject it, and SHALL permit that Mentor to change a decision only under `ATT-024`. THE system SHALL write an immutable correction event for every transition and every decision change. |
...
| COR-009 | WHILE the attendance period of a correction is finalized, THE system SHALL refuse any Mentor state change except inside a range reopened under `ATT-022`. THE system SHALL NOT permit an Admin, or any Mentor other than the responsible Mentor, to decide a correction or change its decision. |
...
| LEV-004 | THE system SHALL reserve quota for `PENDING`, `OVERDUE`, and `APPROVED` leave days, and SHALL release it WHEN a request becomes `REJECTED`, `WITHDRAWN`, or `CANCELLED`, including through a decision change under `ATT-024`. For a selected quota month THE system SHALL show the Intern `reserved / applicable quota / remaining`, where remaining is `max(0, quota − reserved)`. THE system SHALL default the dashboard to the current business month and SHALL permit month selection in My Leave. |
...
| LEV-011 | WHILE the first counted start has not passed, THE system SHALL permit the owning Intern to cancel an approved request, which becomes `CANCELLED`. WHEN leave begins, THE system SHALL freeze the request's date range and its materialized day allocation. WHERE a decision change under `ATT-024` would make a request `APPROVED` again, THE system SHALL revalidate overlap and quota under `LEV-005` and `LEV-006` in the same transaction and SHALL refuse the change when either fails. |
| LEV-012 | THE system SHALL NOT create leave retroactively. WHILE the first counted start of a request has passed, THE system SHALL refuse every change to it except a decision on an `OVERDUE` request under `LEV-008`, a withdrawal under `LEV-013`, and a decision change under `ATT-024`. |
...
| EXC-007 | THE system SHALL permit an exception decision or mark to change only under `ATT-024`. WHERE the current decision changes, THE system SHALL count the violation under `EXC-005` from the new current decision and SHALL leave the classification of `EXC-001` unchanged. |
...
| ATT-024 | WHILE no attendance period it touches is finalized, THE system SHALL permit the Intern's responsible Mentor under `ACC-026` to change a decision on a leave, correction, or attendance exception request, or an exception mark under `EXC-004`, only by appending a new decision or a reversal that carries the actor, the server time, and a nonblank reason. THE system SHALL treat the latest effective decision as the current one, SHALL derive every result from it, and SHALL keep the request and every earlier decision unchanged. WHERE a period it touches is finalized, THE system SHALL refuse the change except inside a range reopened under `ATT-022`. |
...
4. Until the attendance period is finalized, the responsible Mentor may change the decision by appending a new decision or a reversal with a reason.
...
| Trigger | An Intern requests, withdraws, or cancels leave, or the responsible Mentor decides a request or changes a decision. |
...
- Until the period is finalized, the responsible Mentor may change a decision by appending a new decision or a reversal with a reason; approving again revalidates quota and overlap.
...
| AC-COR-003 | COR-004–COR-005, COR-007, ATT-024 | The responsible Mentor approves a correction, two days later reverses it to rejected with a reason before the period is finalized, and then tries a further change without a reason | The approval and the reversal each append an ordered immutable event with actor and time, and the approval stays unchanged in history; no lock applied when the decision window ended; the effective checkout and the missing-checkout classification follow the current rejection; the change without a reason is refused. |
...
| AC-LEV-008 | ATT-024, LEV-004, LEV-011 | Before the period is finalized, the responsible Mentor reverses an approved leave to rejected with a reason; the Intern's quota for that month is then fully reserved by another request, and the Mentor tries to approve the first request again | The reversal appends a rejection with the Mentor, time, and reason, keeps the earlier approval unchanged, and releases the quota; the renewed approval is refused for quota and changes nothing. |
...
- `D14`, decided on 14 September 2026 and **provisional pending instructor confirmation**, covers `EXC-001`–`EXC-007`, leave withdrawal under `LEV-013`, and the monthly periods and shared decision lifecycle of `ATT-019`–`ATT-024`. Enterprise-backed: the fact kept apart from the approval; approval by a responsible Mentor; overdue never meaning rejected; one lifecycle for leave, corrections, and exceptions, in which a decision changes only by an appended decision until its period is finalized and only through reopen afterward; withdrawal by the requester before a decision; reopening decided by an Admin on data-governance grounds only; reassignment when the approver is unavailable. Laboratory policy, not a standard: the 24-hour and 48-hour limits, the three-day finalization grace, and leaving excused violations out of compliance.
...
- **Derived from the shared lifecycle, to confirm.** A reversal replaces the current outcome with the opposite one, and nothing returns a decided request to pending, since pending is not a decision; the earlier revert to `PENDING` in `COR-005` is therefore gone. Because leave shares the lifecycle, the responsible Mentor may reverse an approved leave after its dates, which are then classified without it, and approving again revalidates quota and overlap (`LEV-011`). Withdrawal was given to pending and overdue requests only, so an Intern still cancels an approved request only before its first counted start. Admins are notified of a reopen request and the requester of its outcome (`NOT-011`), following the rule that a request goes to whoever decides it and the outcome to whoever asked.
