...
clarifications are settled elsewhere, so the register is empty and §22.2's condition on it
is met. §22.1 and the header counts move to 310 rules, 291 of them carrying an acceptance
scenario, and 163 scenarios; the 19 rules declared without one are unchanged. `platform`
defines none of the five new rules. An empty register does not establish that the rules are
right for the laboratory, and §22.2 still requires a person to accept them.

## 1.7.3 — 2026-09-21

D37 updates the acceptance catalogue from 155 to 156 scenarios (adding AC-TSK-021), settles Project P3/P4 status constraints and transition tables, and references the assignee non-deleted Task condition in TSK-007. The 305 rules, 286 directly traced rules and 19 declared exclusions remain unchanged in count.
...
D36 updates the acceptance catalogue from 154 to 155 scenarios and references the resolved soft-deleted Task readiness case. The 305 rules, 286 directly traced rules and 19 declared exclusions are unchanged in count; no platform rule changes.
...
D35 updates the catalogue to 305 rules and 154 scenarios, with 286 rules directly traced and 19 declared exclusions. Removes the settled Task/readiness questions; remaining business and technical gaps stay visible. No architecture, security invariant or constitution change.
...
Navigation migration (`D34`). Canonical rules now live in [MODULE.md](../platform/MODULE.md)
...
- The account spec is now the [identity spec](../identity/MODULE.md), and the internship lifecycle has its own [internship spec](../internship/MODULE.md). The policy, the global calendar and HolidayAPI moved from the attendance spec to a new [calendar spec](../calendar/MODULE.md). The task spec merged into the [project spec](../project/MODULE.md). The integration spec was divided by owner and no longer exists.
