...
Navigation cleanup (`D38`). The parallel `feature-reporting/` compatibility index is
...
Organization only (`D34`). Inherits the 1.3.3 baseline from
...
These entries recorded `reporting` while its rules lived in a single `feature-reporting/SPEC.md`.
They are retained verbatim; only their heading depth changed when they moved here.

### 1.4.0 — 2026-09-21

Navigation migration (`D34`). Canonical rules now live in [MODULE.md](../reporting/MODULE.md)
