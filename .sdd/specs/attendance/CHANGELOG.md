...
request's `WITHDRAWN` are undecided and carry no decision, and that a `CANCELLED` leave
request carries the approval it cancelled. `AC-DB-011` covers both; without the first, the
decision checks of V1 would refuse every overdue leave request and correction.
...
These are the entries of the former `feature-attendance/CHANGELOG.md`: the history of that
