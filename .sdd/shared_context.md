...
What anyone working on Lab Timesheet, person or agent, needs to know about the
product and its technology before reading a spec. The rules themselves are in
[`specs/`](specs); this page describes the setting they apply to.
...
- The requirements are the specs under `.sdd/specs/`. They describe a system that is built: four iterations have shipped and the schema lives in Flyway. The specs are approved and currently ahead of the code; `plan.md` tracks the difference.
...
- No production data, testimonials, adoption metrics, endorsements, or performance claims exist. Design work must not invent them.
...
| `CONTRIBUTING.md` | Local setup, running, testing, and the change workflow |
| `README.md` | Production container configuration and operation |
