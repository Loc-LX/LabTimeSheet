...
| ID | Deliverable | Test/evidence emphasis | Status | Owner/date | Result/commit |
|---|---|---|---|---|---|
| I2-UI-01 | Complete Admin/Mentor/Intern/Leader dashboards and notification UI. | Authorization-correct actions and deadline/state summaries. | `IN_PROGRESS` | work/reports-ui / 2026-08-20 | BLOCKED on `I2-PLAT-06` (notification service) and projects exit/readiness queries; dashboards exist from I1 |
| I2-UI-02 | Complete account, integration, Project, invitation, membership-exit, Task, policy/calendar, attendance, correction, and leave desktop workflows using shared fragments. | Leader invite/revoke/removal request, Intern accept/decline/leave/cancel, and Mentor exit decision forms retain safe input and expose conflicts clearly. | `TODO` | work/reports-ui / 2026-08-20 | BLOCKED on `I2-PRJ` (invitation/exit/transfer), `I2-ATT` (correction/leave/policy), `I2-TSK` (work logs), `I2-PLAT` (SMTP/HolidayAPI History) |
| I2-UI-03 | Build one authorized attendance/compliance HTML report dataset. | Date filters, detailed versus own scope, formulas and `N/A`. | `IN_PROGRESS` | work/reports-ui / 2026-08-20 | Formula layer GREEN + page layer GREEN (`docs/tests/web/attendance-report-page.md`); feed integration BLOCKED on attendance classification feed via `AttendanceReportDataProvider` (page 404s until registered) |
| I2-UI-04 | Build one authorized Project/Task HTML report dataset. | Project/member/status/date filters and per-member visibility rules. | `IN_PROGRESS` | work/reports-ui / 2026-08-20 | Formula/visibility layer GREEN + page layer GREEN (`docs/tests/web/project-task-report-page.md`); feed integration BLOCKED on Task work-log / Project membership queries via `ProjectTaskReportDataProvider` (page 404s until registered) |
| I2-UI-05 | Add only meaningful Chart.js trends with adjacent text/table alternatives. | Accessible label, equivalent data, theme tokens, reduced motion. | `IN_PROGRESS` | work/reports-ui / 2026-08-20 | Foundation GREEN (`docs/tests/web/chart-accessibility-and-theme.md`) + wired into both report pages (JSON emitted by `ReportChartJson`); rendering testable only through the pages, which stay 404 until feeds integrate |
...
| 2026-08-20 | `work/reports-ui` | I2-UI-03 shared attendance/compliance report formula layer (rate, daily/period compliance, `N/A`) implemented by strict TDD: RED compile failure, then GREEN; affected reporting/attendance/architecture suites green. Page rendering stays BLOCKED on the Attendance classification feed (absent-day denominator, leave, off-day) per §5.6 integration order. | I2-UI-03 | `docs/tests/unit/attendance-report-formulas.md` (commit pending) | Proceed to I2-UI-04 dataset/formulas (self-contained); later wire I2-UI-03 page when the attendance feed lands. |
| 2026-08-20 | `work/reports-ui` | I2-UI-04 Project/Task report aggregation (progress via `TaskProgress`, minutes, blocked, `N/A`) and per-member visibility decision (Admin/owning Mentor/current Leader) implemented by strict TDD: RED compile failure, then GREEN; affected reporting/task/architecture suites green. Page rendering stays BLOCKED on the Task work-log query for per-member logged minutes per §5.6 integration order. | I2-UI-04 | `docs/tests/unit/project-task-report-formulas.md` (commit pending) | Proceed to I2-UI-05 Chart.js foundation (self-contained); later wire I2-UI-04 page when the work-log feed lands. |
| 2026-08-20 | `work/reports-ui` | I2-UI-05 Chart.js foundation: pinned `chart.js@4.5.1` (exact) + committed lockfile, copy-UMD build step (`build:chart`), and `app-charts.js` initializer reading `data-chart` JSON with theme-token colors, reduced-motion handling, and resilient parse. JS contract RED/GREEN via `npm run test:ui`; `npm run build` produced `chart.umd.js` (208,518 bytes). Chart data wiring stays BLOCKED on the I2-UI-03/04 datasets. | I2-UI-05 | `docs/tests/web/chart-accessibility-and-theme.md` (commit pending) | Proceed to I2-UI-01 dashboards + notification UI; later wire charts into report pages when datasets land. |
| 2026-08-20 | `work/reports-ui` | Self-contained I2-UI-03/04/05 milestones GREEN (formula layers, visibility rules, Chart.js foundation) with evidence. Remaining I2-UI-01/02 surfaces are BLOCKED on `I2-PLAT-06`, `I2-PRJ`, `I2-ATT`, and `I2-TSK` contracts that do not yet exist on this branch; dashboard summaries and notification bell/page will proceed after platform integrates per §5.6 order. | I2-UI-01, I2-UI-02 | — | Continue after `main` integrates platform/projects/tasks/attendance; then wire report pages, charts, notifications, and workflow families. |
| 2026-08-20 | `work/reports-ui` | I2-UI-03 and I2-UI-04 report pages built and tested at the web layer by strict TDD: RED compile failure, then GREEN. `AttendanceReportController` (`/reports/attendance`) and `ProjectTaskReportController` (`/reports/projects`) render real Thymeleaf pages with the real formula services; the dataset boundaries are defined as `AttendanceReportDataProvider` and `ProjectTaskReportDataProvider` resolved through `ObjectProvider`, so the full context still starts and the pages return `404` (no stub data) until the Attendance/Project/Task branches register the providers. Per-member visibility is gated by `allowPerMemberDetail`; charts are emitted by `ReportChartJson` (dependency-free) and only when meaningful. Web tests: 9/9 GREEN; affected reporting/architecture/ui suite 83/83 GREEN. | I2-UI-03, I2-UI-04 | `docs/tests/web/attendance-report-page.md`, `docs/tests/web/project-task-report-page.md` (commit pending) | Integrate the domain feeds behind the two provider interfaces when `I2-ATT`/`I2-TSK`/`I2-PRJ` land; pages and charts then go live automatically. |
